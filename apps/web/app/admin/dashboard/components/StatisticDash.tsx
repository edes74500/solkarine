...
import { useGetCharacterCountQuery } from "@/redux/api/character.apiSlice";
...
export function StatisticDash() {
  ...
  const { data: characterCount } = useGetCharacterCountQuery();
      ...
      <Card className="">
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div
              className="text-center p-4 bg-muted rounded-lg cursor-pointer"
              onClick={() => router.push("/admin/dashboard/characters")}
            >
              <div className="text-2xl font-bold text-primary">{characterCount?.data || 0}</div>
